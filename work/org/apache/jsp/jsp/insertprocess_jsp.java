...
public final class insertprocess_jsp extends org.apache.jasper.runtime.HttpJspBase
...
int quan=0;
  try{
   ...
   String ipd=new String(request.getParameter("ipdate").getBytes("ISO-8859-1"),"UTF-8");
   String na=new String(request.getParameter("nn").getBytes("ISO-8859-1"),"UTF-8");
   String pn=new String(request.getParameter("ph").getBytes("ISO-8859-1"),"UTF-8");
   String tos=new String(request.getParameter("ts").getBytes("ISO-8859-1"),"UTF-8");
   String hno=new String(request.getParameter("hn").getBytes("ISO-8859-1"),"UTF-8");
   String adr=new String(request.getParameter("ad").getBytes("ISO-8859-1"),"UTF-8");
   String typ=new String(request.getParameter("ty").getBytes("ISO-8859-1"),"UTF-8");
   String flo=new String(request.getParameter("fl").getBytes("ISO-8859-1"),"UTF-8");
   String lar=new String(request.getParameter("la").getBytes("ISO-8859-1"),"UTF-8");
   String bar=new String(request.getParameter("ba").getBytes("ISO-8859-1"),"UTF-8");
   String spr=new String(request.getParameter("sp").getBytes("ISO-8859-1"),"UTF-8");
   String rpr=new String(request.getParameter("rp").getBytes("ISO-8859-1"),"UTF-8");
   String par=new String(request.getParameter("pa").getBytes("ISO-8859-1"),"UTF-8");
   String pht=new String(request.getParameter("pto").getBytes("ISO-8859-1"),"UTF-8");
   String rem=new String(request.getParameter("rm").getBytes("ISO-8859-1"),"UTF-8");
   String opd=new String(request.getParameter("opdate").getBytes("ISO-8859-1"),"UTF-8");
   System.out.println("Bring Success");
   String strmycar="INSERT INTO house(inputdate,name,phoneno,township,homeno,address,type,floor,landarea,buildarea,saleprice,rentprice,particular,photo,remark,outputdate) VALUES ('"+ipd+"','"+na+"','"+pn+"','"+tos+"','"+hno+"','"+adr+"','"+typ+"','"+flo+"','"+lar+"','"+bar+"','"+spr+"','"+rpr+"','"+par+"','"+pht+"','"+rem+"','"+opd+"')";
   int RowCount=stmt.executeUpdate(strmycar);   
   System.out.println("Insert Car Success");
  String strname="SELECT * FROM house"; 
  ResultSet rs=stmt.executeQuery(strname); 
  System.out.println("DB connection succeed");
  out.println("<table border=1 style='font-family:Zawgyi-one'>");
  out.println("<tr>");
  ...
  while (rs.next()) 
  {
	   ...
	   out.println("<td style='font-family:Zawgyi-one;'>"+hn+"</td>");
	   ...
	   out.println("<td><img src=/myHouse/image/"+photo+" width=70px height=70px></td>");
